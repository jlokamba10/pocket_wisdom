...
export default function SupervisorSensors() {
  const { token } = useAuth();
  ...
  const loadSensors = async (nextOffset = offset) => {
    ...
    try {
      ...
      const data = await apiRequest<PaginatedSensors>(`/sensors${params}`, { method: "GET" }, token);
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      if (editing) {
        await apiRequest(`/sensors/${editing.id}`, { method: "PUT", body: JSON.stringify(payload) }, token);
        setFormMessage("Sensor updated.");
      } else {
        await apiRequest(`/sensors`, { method: "POST", body: JSON.stringify(payload) }, token);
  ...
  return (
    <section className="page">
      <div className="page-header">
        <h2>Sensors</h2>
        <p>Manage sensors attached to equipment in your scope.</p>
      </div>
      ...
      <div className="panel">
        <div className="table-toolbar">
          ...
          <button className="primary" type="button" onClick={() => { resetForm(); setShowForm(true); }} disabled={equipment.length === 0}>
            New Sensor
          </button>
        </div>
        ...
        {showForm ? (
          <form className="form-grid" onSubmit={onSubmit}>
            ...
            <div className="form-actions">
              <button className="primary" type="submit" disabled={equipment.length === 0}>
            ...
            {equipment.length === 0 ? (
              <div className="form-error">Assign equipment before adding sensors.</div>
            ) : null}
